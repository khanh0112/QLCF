...
namespace QLQuanCafe
{
    public partial class fTableManager : Form
    {
        ...
        void BtnTable_Guna_Click(object sender, EventArgs e)
        {
            ...
            // Reset viền
            foreach (Guna2Button b in flpTable.Controls.OfType<Guna2Button>())
                b.BorderThickness = 0;
            ...
            // Kiểm tra bàn có bill đang mở không
            string checkBill = @"SELECT COUNT(*) 
                         FROM Bill 
                         WHERE IDTable = @tableID AND billStatus = 1";

            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                SqlCommand cmd = new SqlCommand(checkBill, conn);
        ...
        FROM FoodTable ft";
            ...
            using (SqlConnection connection = new SqlConnection(connection_string_sql))
            {
                ...
                while (reader.Read())
                {
                    ...
                    btn.ShadowDecoration.Color = Color.FromArgb(50, 0, 0, 0);
                    ...
                    if (isOccupied)
                    {
                        ...
                        btn.FillColor = Color.FromArgb(239, 68, 68);   // đỏ nhạt
                        btn.FillColor2 = Color.FromArgb(220, 38, 38); // đỏ đậm
                    ...
                    else
                    {
                        ...
                        btn.FillColor = Color.FromArgb(34, 197, 94);  // xanh nhạt
                        btn.FillColor2 = Color.FromArgb(22, 163, 74); // xanh đậm
                        btn.ForeColor = Color.White;
                    }
