    ...
    public partial class fTableManager : Form
    {
        ...
        void LoadCategory()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                ...
                cbCategory1.DataSource = dt;
                cbCategory1.DisplayMember = "foodCateName";
                cbCategory1.ValueMember = "foodCateID";
            }
            cbCategory1.SelectedIndex = 0;
            cbFood1.DataSource = null; // reset food
        }
        ...
        void LoadFoodByCategory(int cateID) // load món theo danh mục
        {
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                ...
                cbFood1.DataSource = dt;
                cbFood1.DisplayMember = "foodName";
                cbFood1.ValueMember = "foodID";
            }
        }
        ...
        {
            if (cbCategory1.SelectedValue == null) return;
            if (!(cbCategory1.SelectedValue is int)) return;

            int cateID = (int)cbCategory1.SelectedValue;

            if (cateID == -1)
            {
                cbFood1.DataSource = null; // chưa chọn danh mục
                return;
            }
        ...
        private void btnAddFood_Click(object sender, EventArgs e) // thêm món vào bill
        {
            ...
            if (cbFood1.SelectedValue == null) return;

            int foodID = (int)cbFood1.SelectedValue;
            int quantity = (int)nmFoodCount1.Value;
        ...
        private void btnUpdateFood_Click(object sender, EventArgs e) // cập nhật số lượng món
        {
            ...
            int newQty = (int)nmFoodCount1.Value;
            if (newQty <= 0)
            {
                MessageBox.Show("Số lượng phải lớn hơn 0");
                return;
        ...
        private void btnDelFood_Click(object sender, EventArgs e)
        {
            ...
            selectedFoodID = -1;
            originalQuantity = 0;
            nmFoodCount1.Value = 0;
            cbFood1.DataSource = null;
        ...
        void LoadFoodInfoByName(string foodName) // load thông tin món theo tên món
        {
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                ...
                if (rd.Read())
                {
                    ...
                    cbCategory1.SelectedValue = cateID;
                    LoadFoodByCategory(cateID);
                    cbFood1.SelectedValue = selectedFoodID;
                }
            }
        }
        ...
        void FoodField_Changed(object sender, EventArgs e) // theo dõi thay đổi số lượng món
        {
            ...
            bool changed =
                (int)nmFoodCount1.Value != originalQuantity;

            btnUpdateFood1.Text = changed ? "Cập nhật" : "Sửa món";
