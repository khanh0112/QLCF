...
namespace QLQuanCafe
{
    public partial class fTableManager : Form
    {
        ...
        void LoadTable() // load danh sách bàn
        {
        ...
        private void btnDelFood_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                ...
                //Xóa món
                SqlCommand del = new SqlCommand(@"DELETE FROM BillInfo WHERE IDBill = @billID AND IDFood = @foodID", conn);
            ...
            SELECT COUNT(*) FROM BillInfo WHERE IDBill = @billID
        ", conn);
                ...
                if (count == 0)
                {
                    SqlCommand closeBill = new SqlCommand(@"
                UPDATE Bill SET billStatus = 0 WHERE billID = @billID
            ", conn);
        ...
        void LoadFoodInfoByName(string foodName)
        {
            string query = @"
        SELECT f.foodID, f.foodName, f.IDCategory
        FROM Food f
        WHERE f.foodName = @name
    ";
