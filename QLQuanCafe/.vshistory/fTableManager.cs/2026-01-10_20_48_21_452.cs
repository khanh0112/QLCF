...
namespace QLQuanCafe
{
    public partial class fTableManager : Form
    {
        ...
        void HighlightCurrentTable() // tô màu bàn đang chọn
        {
            foreach (Button b in flpTable.Controls.OfType<Button>())
            {
                int tableID = (int)b.Tag;
                bool isActive = tableID == currentTableID;

                if (isActive)
                {
                    b.BackColor = colorActive;
        ...
        void LoadTable() // load bàn
        {
            ...
            using (SqlConnection connection = new SqlConnection(connection_string_sql))
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (isOccupied)
                    {
                        btn.BackColor = colorOccupied;
                        btn.Text = tableName
                            + Environment.NewLine + "Có người"
                            + Environment.NewLine + total.ToString("N0") + " VND";
                    }
                    else
                    {
                        btn.BackColor = colorEmpty;
                        btn.Text = tableName
                            + Environment.NewLine + "Trống";
                    }

                    btn.ForeColor = Color.FromArgb(31, 41, 55); // chữ xám đậm sang
