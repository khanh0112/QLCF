...
namespace QLQuanCafe
{
    public partial class fAdmin : Form
    {
        ...
        private void btnDelAccount_Click(object sender, EventArgs e)
        {
            ...
            string qCheck = "SELECT COUNT(*) FROM Account WHERE userName = @userName";
            ...
            DialogResult rs = MessageBox.Show("Bạn có chắc muốn xóa tài khoản này?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);

            if (rs == DialogResult.No) return;

            string qDel = "DELETE FROM Account WHERE userName = @userName";
            boDocGhi.DeleteCommand = new SqlCommand(qDel, ketNoi);
            try
            {
                ketNoi.Open();
                int kq = (int)boDocGhi.DeleteCommand.ExecuteNonQuery();
                if (kq != 0)
                {
